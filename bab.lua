-- ==========================================
-- SOL'S RNG TRACKER V10.14 (ULTIMATE CHAMELEON & CUSTOM ICONS)
-- ==========================================
...
-- ==========================================
-- TÙY CHỈNH ẢNH CHO TỪNG BIOME & STYLE
-- ==========================================
...
local DEFAULT_MERCHANT_ICON = "https://static.wikia.nocookie.net/sol-rng/images/8/87/Icon.png"
...
-- ==========================================
-- DISCORD WEBHOOK HANDLER (CHAMELEON STYLES)
-- ==========================================
local function sendDiscordEmbed(eventType, name, isEnd)
    ...
    local macroIconUrl = DEFAULT_MERCHANT_ICON
    local defaultServerDisplay = (psLink ~= "") and ("[Private Server Link](" .. psLink .. ")") or ("`" .. jobId .. "`")

    -- COLOR, PING, AND ICON LOGIC
    if eventType == "Merchant" then
        if name == "Mari" then colorHex = 0xFF66A3; pingContent = getgenv().PingMari or ""
        elseif name == "Rin" then colorHex = 0xE67E22; pingContent = getgenv().PingRin or ""
        elseif name == "Jester" then colorHex = 0x9B59B6; pingContent = getgenv().PingJester or "" end
    elseif eventType == "Biome" then
        if BIOME_VISUALS[name] then 
            colorHex = BIOME_VISUALS[name].Color 
            macroIconUrl = (BIOME_VISUALS[name].MacroIcon ~= "") and BIOME_VISUALS[name].MacroIcon or DEFAULT_MERCHANT_ICON
        end
    ...
    -- STYLE 1: EggSol (FishSol)
    if style == "EggSol" then
        local stateText = isEnd and "Ended" or "Started"
        local titlePrefix = eventType == "Merchant" and "Merchant Arrived" or ("Biome " .. stateText)
        
        embed.title = titlePrefix .. " - " .. name
        if not isEnd and psLink ~= "" then
            embed.description = "[Join Server](" .. psLink .. ")"
        end
        embed.color = colorHex
        
        if not (eventType == "Biome" and isEnd) then
            embed.thumbnail = { ["url"] = eventType == "Merchant" and DEFAULT_MERCHANT_ICON or macroIconUrl }
        end
        
        embed.footer = { ["text"] = "EggSol v1.9.7" }
        embed.timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")

    -- STYLE 2: DroidScope
    elseif style == "DroidScope" then
        local stateText = isEnd and "Ended" or "Started"
        local titlePrefix = eventType == "Merchant" and "Merchant Arrived" or ("Biome " .. stateText)
        
        embed.title = titlePrefix .. " - " .. name
        local desc = "<t:" .. unixTime .. ":T> ( <t:" .. unixTime .. ":R> )"
        ...
        -- DroidScope ẨN ẢNH KHI BIOME ENDED
        if not (eventType == "Biome" and isEnd) then
            embed.thumbnail = { ["url"] = eventType == "Merchant" and DEFAULT_MERCHANT_ICON or macroIconUrl }
        end
        
        embed.footer = { ["text"] = "DroidScope | Release v1.0.0" }

    -- STYLE 3: Maxstellar
    elseif style == "Maxstellar" then
        local stateText = isEnd and "Ended" or "Started"
        local titlePrefix = eventType == "Merchant" and "Merchant Arrived" or ("Biome " .. stateText)
        
        embed.author = { ["name"] = "[" .. os.date("%H:%M:%S") .. "]" }
        embed.title = titlePrefix .. " - " .. name
        
        if not isEnd and psLink ~= "" then
            embed.description = "**Private Server Link**\n" .. psLink
        end
        embed.color = colorHex
        
        -- Maxstellar GIỮ ẢNH KỂ CẢ KHI ENDED
        embed.thumbnail = { ["url"] = eventType == "Merchant" and DEFAULT_MERCHANT_ICON or macroIconUrl }
        
        embed.footer = { 
            ["text"] = "maxstellar's Biome Macro | v2.3", 
            ["icon_url"] = "https://i.imgur.com/B9P8bO7.png" 
        }

    -- STYLE 4: J.JARAM (Default)
    else
        ...
        if eventType == "Merchant" then
            local icon = "🛒"
            if name == "Mari" then icon = "🛍️" elseif name == "Rin" then icon = "🦊" elseif name == "Jester" then icon = "🃏" end
        ...
        embed.color = colorHex
        embed.footer = { ["text"] = "J.JARAM JX 2x56 • " .. serverId }
        embed.timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
        if thumbnailUrl then embed.thumbnail = { ["url"] = thumbnailUrl } end
    end

    local payload = { ["content"] = (pingContent ~= "") and pingContent or nil, ["embeds"] = {embed} }
...
local function triggerBiomeChange(newBiome)
    ...
    -- LẤY STYLE HIỆN TẠI TỪ GETGENV() (MẶC ĐỊNH LÀ J.JARAM NẾU KHÔNG CÓ)
    local style = getgenv().MacroStyle or "J.JARAM"
    local hideNormal = (style == "J.JARAM") -- ẨN NORMAL NẾU LÀ J.JARAM
    
    -- GỬI "ENDED"
    if oldBiome ~= "" then 
        if not hideNormal or oldBiome ~= "NORMAL" then
    ...
    if currentBiome ~= "" then
        if not hideNormal or currentBiome ~= "NORMAL" then
...
local function performBiomeCheck()
    ...
    if workspaceBiome and workspaceBiome:IsA("StringValue") then
        wsBiomeValue = parseBiome(workspaceBiome.Value, true)
    end
